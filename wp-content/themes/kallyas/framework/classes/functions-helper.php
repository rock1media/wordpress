...
function zn_extract_link( $link_array , $class = false , $attributes = false, $def_start = '', $def_end = '', $def_url = false ){
	...
	if ( !is_array( $link_array ) || empty( $link_array['url'] ) ) {
		...
		$link = array( 'start' => '<a href="'.esc_url( $link_array['url'] ).'" '.$attributes.' class="'.$class.'" title="'.esc_attr( $link_array['title'] ).'" '.zn_get_target( esc_attr( $link_array['target'] ) ).'>' , 'end' => '</a>' );
	}
...
/*--------------------------------------------------------------------------------------------------
	...
	@returns : string
--------------------------------------------------------------------------------------------------*/
function zn_extract_link_title( $link_array ){

	return is_array( $link_array ) && !empty( $link_array['title'] ) ? esc_attr( $link_array['title'] ) : '';
